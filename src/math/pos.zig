...
pub const Pos = struct {
    ...
    pub fn randFromPos(self: Pos) f32 {
        ...
        return @as(f32, @floatFromInt(result & 0xFFFFFFFF)) / 4294967295.0;
    }

    pub fn distanceTiles(self: Pos, other: Pos) i32 {
        const dist_x: i32 = @intCast(@abs(self.x - other.x));
        const dist_y: i32 = @intCast(@abs(self.y - other.y));
    ...
    pub fn distance(self: Pos, other: Pos) f32 {
        const x_dist = @as(f32, @floatFromInt(self.x - other.x));
        const y_dist = @as(f32, @floatFromInt(self.y - other.y));
        return @abs(std.math.sqrt(x_dist * x_dist + y_dist * y_dist));
    }
