...
pub const AsciiTexture = struct {
    ...
    pub fn renderAsciiCharacters(renderer: *Renderer, font: *Font) !AsciiTexture {
        sdl3.TTF_SetFontStyle(font, sdl3.TTF_STYLE_BOLD);
        ...
        const text_surface = sdl3.TTF_RenderText_Blended(font, txt.ptr, txt.len, Sdl2Color(Color.white()));
        defer sdl3.SDL_DestroySurface(text_surface);

        const font_texture = sdl3.SDL_CreateTextureFromSurface(renderer, text_surface) orelse {
            sdl3.SDL_Log("Unable to create sprite texture: %s", sdl3.SDL_GetError());
            return error.SDLInitializationFailed;
        };

        var w: f32 = undefined;
        var h: f32 = undefined;
        _ = sdl3.SDL_GetTextureSize(font_texture, &w, &h);
...
pub fn processTextGeneric(canvas: Canvas, text: [128]u8, len: usize, color: Color, pixel_pos: Pos, scale: f32) void {
    ...
    _ = sdl3.SDL_SetTextureBlendMode(canvas.target, sdl3.SDL_BLENDMODE_BLEND);
    _ = sdl3.SDL_SetTextureColorMod(canvas.ascii_texture.texture, color.r, color.g, color.b);
    _ = sdl3.SDL_SetTextureAlphaMod(canvas.ascii_texture.texture, color.a);
    ...
    for (text[0..len]) |chr| {
        ...
        _ = sdl3.SDL_RenderTextureRotated(canvas.renderer, canvas.ascii_texture.texture, &Sdl3Rect(src_rect), &Sdl3Rect(dst_rect), 0.0, null, 0);
...
pub fn processTextJustify(canvas: Canvas, params: drawing.drawcmd.DrawTextJustify) void {
    ...
    if (params.bg_color) |bg_color| {
        _ = sdl3.SDL_SetTextureBlendMode(canvas.target, sdl3.SDL_BLENDMODE_NONE);
        ...
        _ = sdl3.SDL_SetRenderDrawColor(canvas.renderer, bg_color.r, bg_color.g, bg_color.b, bg_color.a);
        _ = sdl3.SDL_RenderFillRect(canvas.renderer, &Sdl3Rect(rect));
    }
    _ = sdl3.SDL_SetTextureBlendMode(canvas.target, sdl3.SDL_BLENDMODE_BLEND);
...
pub fn processFillCmd(canvas: Canvas, params: drawing.drawcmd.DrawFill) void {
    const cell_dims = canvas.panel.cellDims();
    _ = sdl3.SDL_SetRenderDrawColor(canvas.renderer, params.color.r, params.color.g, params.color.b, params.color.a);
    ...
    var sdl3_rect = Sdl3Rect(src_rect);
    _ = sdl3.SDL_RenderFillRect(canvas.renderer, &sdl3_rect);
}
...
pub fn processRectCmd(canvas: Canvas, params: drawing.drawcmd.DrawRect) void {
    ...
    _ = sdl3.SDL_SetRenderDrawBlendMode(canvas.renderer, sdl3.SDL_BLENDMODE_BLEND);
    _ = sdl3.SDL_SetRenderDrawColor(canvas.renderer, params.color.r, params.color.g, params.color.b, params.color.a);
    ...
    if (params.filled) {
        _ = sdl3.SDL_RenderFillRect(canvas.renderer, &Sdl3Rect(Rect.initAt(x, y, width, height)));
    } else {
        const size = @divFloor(@divFloor(canvas.panel.num_pixels.width, canvas.panel.cells.width), 10);
        _ = sdl3.SDL_RenderFillRect(canvas.renderer, &Sdl3Rect(Rect.initAt(x, y, size, height)));
        _ = sdl3.SDL_RenderFillRect(canvas.renderer, &Sdl3Rect(Rect.initAt(x, y, width, size)));
        _ = sdl3.SDL_RenderFillRect(canvas.renderer, &Sdl3Rect(Rect.initAt(x + width, y, size, height + size)));
        _ = sdl3.SDL_RenderFillRect(canvas.renderer, &Sdl3Rect(Rect.initAt(x, y + height, width + size, size)));
    }
}
...
pub fn processRectFloatCmd(canvas: Canvas, params: drawing.drawcmd.DrawRectFloat) void {
    ...
    if (params.filled) {
        _ = sdl3.SDL_RenderFillRect(canvas.renderer, &Sdl3Rect(Rect.initAt(x_offset, y_offset, width, height)));
    } else {
        _ = sdl3.SDL_RenderFillRect(canvas.renderer, &Sdl3Rect(Rect.initAt(x_offset, y_offset, size, height)));
        _ = sdl3.SDL_RenderFillRect(canvas.renderer, &Sdl3Rect(Rect.initAt(x_offset, y_offset, width + size, size)));
        _ = sdl3.SDL_RenderFillRect(canvas.renderer, &Sdl3Rect(Rect.initAt(x_offset + width, y_offset, size, height)));
        _ = sdl3.SDL_RenderFillRect(canvas.renderer, &Sdl3Rect(Rect.initAt(x_offset, y_offset + height - size, width + size, size)));
    }
}

pub fn Sdl2Color(color: Color) sdl3.SDL_Color {
    return sdl3.SDL_Color{ .r = color.r, .g = color.g, .b = color.b, .a = color.a };
}

pub fn Sdl3Rect(rect: Rect) sdl3.SDL_FRect {
